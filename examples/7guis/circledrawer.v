...
fn (mut app App) click_circles(c &ui.CanvasLayout, e ui.MouseEvent) {
	mut sw := c.ui.window.get_widget_by_id_or_panic[ui.SubWindow]('sw_radius')
	if c.ui.btn_down[0] {
		if sw.is_visible() {
			sw.set_visible(false)
			sl := c.ui.window.get_widget_by_id_or_panic[ui.Slider]('sl_radius')
	...
	mut btn_undo := c.ui.window.get_widget_by_id_or_panic[ui.Button]('btn_undo')
	check_undo_disabled(app.state, mut btn_undo)
	mut btn_redo := c.ui.window.get_widget_by_id_or_panic[ui.Button]('btn_redo')
...
fn (mut app App) click_undo(mut b ui.Button) {
	...
	mut redo := b.ui.window.get_widget_by_id_or_panic[ui.Button]('btn_redo')
...
fn (mut app App) click_redo(mut b ui.Button) {
	...
	mut undo := b.ui.window.get_widget_by_id_or_panic[ui.Button]('btn_undo')
